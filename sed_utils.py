...
# Part 1 - functions used for initial data exploration
...
def tracks_content_info(meta_track, meta_segments, dataset_type, track_dur, event_type):
    ...
    print('Total event minutes:', total_time_event_min)
    print('Total neg event minutes:', total_time_neg_min)
...
def segments_dicts_est(predictions_matrix, meta_track, track_dur = 10.0, Ty = 212):
    '''
...
def plot_pred_vs_true(meta_track, actual_labels, raw_predictions, dataset,
    ...
    l1,=ax.plot(x_axis_vals, y_test, color='#e6005c')
    l2,=ax.plot(x_axis_vals, y_pred, color='#a6a6a6', alpha=0.4)
    ax.fill_between(x_axis_vals, 0, y_pred, color='#a6a6a6', alpha=0.4)
    ...
    ax.spines["bottom"].set_edgecolor('#a6a6a6')
    ax.spines["left"].set_edgecolor('#a6a6a6')
    ax.tick_params(length=2, width=0.5, colors='0.4',labelsize = 12)
    leg = plt.legend([l1, l2],["ground truth", "prediction"],
